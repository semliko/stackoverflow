...
RSpec.describe QuestionsController, type: :controller do
  ...
  describe 'DELETE #delete_attached_file' do
    context 'as an author of the question' do
      ...
      let!(:question) { create(:question, user: user_1, files: [file_1]) }
      it 'deletes the answer' do
        file = question.files.first
        expect { delete :delete_attached_file, params: { id: question, file_id: file.id } }.to change(question.files, :count).by(-1)
      end
    end
  end
  ...
  describe 'DELETE #delete_attached_file' do
    context 'as NOT an author of the question' do
      ...
      it 'cannot delete the answer' do
        ...
        login(user_2)
        file = question.files.first
        expect { delete :delete_attached_file, params: { id: question, file_id: file.id } }.to change(question.files, :count).by(0)
      end
    end
  end
end
